...
def startcd():
    ...
    def update():
        ...
        if userinput > -1:
            ...
            hour_tf.config(state='disabled')
            mins_tf.config(state='disabled')
            sec_tf.config(state='disabled')
    update( )
...
imgps = tk.Button(tab_clock, image=render02, borderwidth=0)
imgps.image = render02
imgps.place(x=485,y=500)
reset = Image.open(os.path.join(PATH_IMAGES, "resetbtn.png"))
render03 = ImageTk.PhotoImage(reset)
imgrs = tk.Button(tab_clock, image=render03, borderwidth=0,command=reset)
